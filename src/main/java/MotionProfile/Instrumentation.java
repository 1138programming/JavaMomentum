...
public class Instrumentation {
	...
	public static void OnUnderrun() {
		System.out.format("%s\n", "UNDERRUN");
		SmartDashboard.putString("UNDERRUN", "UNDERRUN"); 
	}

	public static void OnNoProgress() {
		System.out.format("%s\n", "NOPROGRESS");
		SmartDashboard.putString("NOPROGRESS", "NOPROGRESS"); 
	}
	...
	public static void process(MotionProfileStatus status, double pos,
		...
		if ((now - timeout) > 0.2) {
			...
			SmartDashboard.putString("outEn", StrOutputEnable(status.outputEnable));
			SmartDashboard.putNumber("topCnt", status.topBufferCnt);
			SmartDashboard.putNumber("topRem", status.topBufferRem);
			SmartDashboard.putNumber("btmCnt", status.btmBufferCnt);
			SmartDashboard.putBoolean("IsValid", (status.activePointValid));
			SmartDashboard.putBoolean("HasUnder", (status.hasUnderrun));
			SmartDashboard.putBoolean("IsUnder", (status.isUnderrun));
			SmartDashboard.putBoolean("IsLast", (status.isLast));
			SmartDashboard.putNumber("POS", pos);
			SmartDashboard.putNumber("VEL", vel);
			SmartDashboard.putNumber("SlotSel0", status.profileSlotSelect);
			SmartDashboard.putNumber("timeDurMs", status.timeDurMs);
		}
	}
}